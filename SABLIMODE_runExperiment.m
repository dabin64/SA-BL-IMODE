...
for BI = BI_list'
    ...
    for runNo = 1:maxRuns
        ...
        [groups,groupNum] = group(BI);
        ins = SABLIMODE(BI,groups,groupNum);
		ins.runPath = pwd;
		ins.runNo = runNo;
		ins.BI = BI;
		ins.alg = algName;
%         fprintf('%s %s #%d [%g,%g] %d %d %d %d  \n', ins.alg, ins.BI.fn, ins.runNo, ins.UF, ins.LF,ins.UFEs,ins.LFEs,ins.UX,ins.LX);
        ...
        all_output_data(t,1)=ins.UF-BI.u_fopt;
        all_output_data(t,2)=ins.LF-BI.l_fopt;
        all_output_data(t,3)=ins.UFEs;
        all_output_data(t,4)=ins.LFEs;
        all_output_data(t,5)=toc;
    end	
    j=j+1;
       for y=1:5
       all_output_data1(j,2*y-1)=prctile(all_output_data((j-1)*20+1:20*j,y),50);
       all_output_data1(j,2*y)=prctile(all_output_data((j-1)*20+1:20*j,y),75)-prctile(all_output_data((j-1)*20+1:20*j,y),25);
       end
end
xlswrite('SA-BL-IMODE-5-IQR.xls',all_output_data1);
save('SA-BL-IMODE-5.mat','all_output_data');
